...
class QrCode:
    ...
    @staticmethod
    def render_qr_to_image(qr: QrCode, scale: int = 10, border: int = 4) -> Image.Image:
        """Convierte un objeto QrCode en una imagen Pillow."""
        size = qr.get_size()
        ...
        for y in range(size):
            for x in range(size):
                color = 0 if qr.get_module(x, y) else 255
...
# Exception class used in encode_segments()
class DataTooLongError(ValueError):
    pass


# --- Código principal para generar el QR ---

if __name__ == "__main__":
    # URL que deseas codificar
    url = "https://www.eafit.edu.co/"

    # Generar código QR usando la clase QrCode definida arriba
    ...
    img.save("eafit_qr.png")
    print("QR guardado como 'eafit_qr.png'")
